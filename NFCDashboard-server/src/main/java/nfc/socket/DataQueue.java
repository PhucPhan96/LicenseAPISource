...
//        }
    ...
    public void sendPendingData(String storeId){
		
        LinkedList<StoreBasePacket> list = pendingDataMap.get(storeId);
        if (list == null) return;

        while(!list.isEmpty()){
            StoreBasePacket data = list.removeFirst();
            System.out.println("pending : " + data);
            sendDataToClient(storeId, data);
        }
    }    
    
    public void addPendingData(StoreBasePacket msg){
		
        LinkedList<StoreBasePacket> list = pendingDataMap.get(msg.getStoreId());
        if (list == null){
        ...
        list.add(msg);
        pendingDataMap.put(msg.getStoreId()+"", list);
        ...
        public void run() {
            while(true){
                synchronized(dataQueue){
                    ...
                    System.err.println("Chay vao day ne");
                    StoreBasePacket order = (StoreBasePacket)dataQueue.removeFirst();
                    List<String> keys = keymap.getKey(order.getStoreId()+"");
                    System.err.println("Get keys " + keys.size());
                    if(keys!=null && keys.size() > 0){
                        System.err.println("Vao send pending");
                        sendPendingData(order.getStoreId()+ "");
                        sendDataToClient(order.getStoreId()+ "", order);
                    }
                    else{
